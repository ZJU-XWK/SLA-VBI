function [NMSE, sigPar] = IFSLA_VBI_simplify(sigPar, Max_iter)
...
%                  MuX = step_size*noise_precision.*(FHY-HessX*MuX+Lx(iter_x).*MuX);
            ...
            bnoise_tilde=bnoise+y'*y-2*real(MuX'*F'*y)+MuX'*HessX*MuX+sum(diag(HessX).*diag(SigmaX));   
        ...
        Index_set = find(abs(MuTheta)>1e-3);
        MuTheta_temp = zeros(nGrid,1);
        SigmaTheta_temp = zeros(nGrid,1);
        MuTheta_temp(Index_set) = MuTheta(Index_set);
        SigmaTheta_temp(Index_set) = SigmaTheta(Index_set);
        MuTheta = MuTheta_temp;
        SigmaTheta = SigmaTheta_temp;
    end
    sigPar{1}.x_post = MuX;
