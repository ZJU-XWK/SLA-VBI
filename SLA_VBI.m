function [NMSE, sigPar] = SLA_VBI(sigPar, Max_iter)
...
%     noise_precision=1/sigPar{1}.sigma2;
    ...
    for iter = 1:Max_iter
        ...
        if (iter == 1)
            ...
            HessX = F'*F+H_theta.*SigmaTheta;
            iterations = 1;
        end    
         % update for q(x)
        for i = 1:iterations
            if (iter==1)
                SigmaX = (noise_precision*(F'*F)+diag(a_tilde./b_tilde))^-1;
...
%                 MuX = SigmaX*noise_precision*(F'*y+0.5*diag(diag(SigmaTheta))*G_theta*y+0.5*diag(diag(SigmaTao))*G_tao*y);
        ...
        % update for q(theta)
        SigmaTheta = (noise_precision*Hess_theta+0.5*eye(nGrid))^-1;
